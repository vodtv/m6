...
// 重新导出类型以保持API兼容性
export type { PlayRecord, Favorite } from './types';
...
function checkShouldUpdateOriginalEpisodes(
  ...
  const shouldUpdate =
    hasWatchedBeyondOriginal && hasMoreEpisodes && hasSignificantProgress;

  if (shouldUpdate) {
    console.log(
      `检测到应更新原始集数: ${existingRecord.title} - 观看到第${newRecord.index}集，超过原始${originalEpisodes}集，当前总${newRecord.total_episodes}集`
    );
  }
